...
const getValidationSchema = (type: 'customer' | 'staff') => {
  let schema = {
    fullName: yup.string()
...
export default function ReusableSignup({ type }: ReusableSignupProps) {
    ...
    try {
      ...
      if (isStaff) {
        ...
        if (response.succeeded) {
          alert(response.message || "Staff registration successful! You can now log in.");
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100 px-4 py-12 flex-col">
      {/* Terms Modal */}
      ...
      <Card className="w-full max-w-md shadow-xl mb-5">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-[#6B8A62]">
            {isStaff ? 'Staff Registration' : 'Create an account'}
          </h1>
          <p className="mt-2 text-sm text-gray-600">
            {isStaff 
              ? 'Use your invitation code to join as staff' 
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div>
            ...
            <TextInput
              id="phoneNumber" name="phoneNumber" type="tel" icon={HiPhone} placeholder="+1 234 567 8900"
          ...
          {isStaff && (
            <div>
              ...
              <TextInput
                id="invitationCode" name="invitationCode" icon={HiKey} placeholder="Enter your invitation code"
                value={formData.invitationCode || ''} onChange={handleChange} onBlur={handleBlur}
                color={getError('invitationCode') ? 'failure' : 'gray'} disabled={isSubmitting}
              />
              {getError('invitationCode') && <p className="text-xs text-red-600 mt-1">{getError('invitationCode')}</p>}
            </div>
          )}
          ...
          <button type="submit" disabled={isSubmitting}
            ...
            {isSubmitting ? (
              <span className="flex items-center justify-center gap-2">
                ...
                {isStaff ? 'Creating Staff Account...' : 'Creating Account...'}
              </span>
            ) : (
          ...
          {!isStaff && (
            <>
              <p className="text-center text-sm text-gray-600">
                Continue as a guest? <Link to={APP_ROUTES.home} className="text-[#6B8A62] hover:underline">Guest</Link>
              </p>
              <p className="text-center text-sm text-gray-600 font-semibold">
                Registering as a Staff Member? <Link to={APP_ROUTES.staffRegister} className="text-white hover:underline bg-[#6B8A62] inline-block px-3 py-1 rounded-full">Yes</Link>
              </p>
            </>
          )}
          ...
          {isStaff && (
            <p className="text-center text-sm text-gray-600 font-semibold">
              Are you a regular customer? <Link to={APP_ROUTES.signup} className="text-white hover:underline bg-[#6B8A62] inline-block px-3 py-1 rounded-full">Yes</Link>
            </p>
          )}
        </form>
      </Card>
